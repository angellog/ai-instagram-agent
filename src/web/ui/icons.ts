/**
 * Inline SVG icons (Lucide geometry, stroke 2, 24px grid). One icon language
 * across the console; no emoji as UI icons.
 */
const P: Record<string, string> = {
  ...
  sparkles: '<path d="M9.94 14.06 8 20l-1.94-5.94L0 12l6.06-1.94L8 4l1.94 6.06L16 12z" transform="translate(2 -1) scale(.9)"/><path d="M19 3v4M17 5h4M20 16v3M18.5 17.5h3"/>',
  ...
  images: '<path d="M18 22H4a2 2 0 0 1-2-2V6"/><path d="m22 13-1.3-1.3a2.4 2.4 0 0 0-3.4 0L12 17"/><circle cx="12" cy="8" r="2"/><rect x="6" y="2" width="16" height="16" rx="2"/>',
...
export function icon(name: keyof typeof P | string, size = 18, label?: string): string {
  const body = P[name] ?? P.info;
  const a11y = label ? `role="img" aria-label="${label.replace(/"/g, "")}"` : 'aria-hidden="true"';
  return `<svg class="i" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ${a11y}>${body}</svg>`;
}
