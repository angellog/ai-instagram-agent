...
export async function sweep(now = new Date()): Promise<Record<string, number>> {
  ...
  const stuckInteractions = await many<{ id: number }>(
  ...
  for (const r of stuckInteractions) {
    await queue("conversation").add(JOBS.conversationProcess, { interactionId: r.id }, { jobId: jobId("interaction", r.id, "sweep", bucket) });
    out.interactions++;
  }

  const stuckProduction = await many<{ id: string }>(
    `SELECT id FROM posts WHERE status IN ('draft', 'generating', 'composing') AND updated_at < now() - interval '45 minutes' LIMIT 20`,
  );
  for (const r of stuckProduction) {
    await queue("content").add(JOBS.contentProduce, { postId: r.id }, { jobId: jobId("produce", r.id, "sweep", bucket) });
    out.production++;
  }

  const duePublish = await many<{ id: string }>(
  ...
  for (const r of duePublish) {
    await queue("publish").add(JOBS.postPublish, { postId: r.id }, { jobId: jobId("publish", r.id, "sweep", bucket) });
    out.publishing++;
  }
